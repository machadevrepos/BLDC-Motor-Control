...
#define DPGA_IP_AR_RELEASE_MINOR_VERSION_C       9
#define DPGA_IP_AR_RELEASE_REVISION_VERSION_C    0
#define DPGA_IP_SW_MAJOR_VERSION_C               7
                                                                      ...
                                                                      const uint8 OffsetSel)
{
    ...
    uint16 AdcOffsetValue = (uint16)(((OffsetArray[OffsetSel]*DPGA_IP_SELFTEST_ADC_MAX_VALUE)/DPGA_IP_MAX_REFERENCE_VOLTAGE) & 0xFFFFU);
    uint32 Deviation = 0U;
    ...
    {
        TargetValue = (uint32)DataNoOffset + ((uint32)AdcOffsetValue*(uint32)GainSelArray[GainSel]);
    ...
    if (TargetValue < (uint32)DataWithOffset)
    {
        Deviation = (uint32)DataWithOffset - TargetValue;
    }
    else
    {
        Deviation = TargetValue - (uint32)DataWithOffset;
    }
...
Dpga_Ip_StatusType Dpga_Ip_IRQHandler(const uint8 Instance)
{
    ...
    /* CPR_RTD_00012: If the driver is not initialized, the ISR shall clear the interrupt status flag only for the enabled interrupts and return immediately.*/
