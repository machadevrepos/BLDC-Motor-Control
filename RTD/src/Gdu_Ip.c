...
#define GDU_IP_AR_RELEASE_MINOR_VERSION_C       9
#define GDU_IP_AR_RELEASE_REVISION_VERSION_C    0
#define GDU_IP_SW_MAJOR_VERSION_C               7
...
Gdu_Ip_StatusType Gdu_Ip_Init (const Gdu_Ip_ConfigType * const Configuration)
{
    ...
    ControlRegister &= (uint16)(~(GDU_AE_CTL_BOOSTEN_MASK | GDU_AE_CTL_CPEN_MASK) & 0xFFFFU);
...
Gdu_Ip_StatusType Gdu_Ip_SetMode (Gdu_Ip_ModeType Mode)
{
    ...
    switch (Mode)
    {
        ...
        case GDU_IP_MODE_DISABLE:
            ...
            ControlRegister &= (uint16)(~(GDU_AE_CTL_GDUEN_MASK | GDU_AE_CTL_CPEN_MASK) & 0xFFFFU);
        break;
...
Gdu_Ip_StatusType Gdu_Ip_SetSafeState (Gdu_Ip_SafeStateType SafeState)
{
    ...
    switch (SafeState)
    {
        ...
        case GDU_IP_SAFESTATE_OFF:
            ControlRegister &= (uint16)(~GDU_AE_CTL_SSTEN_MASK & 0xFFFFU); /* disable safe state */
        break;
...
void Gdu_Ip_ISR (uint8 interruptMode)
{
    ...
    if (interruptMode < 8u)
    {
        tempMask = (uint32)1u << interruptMode;
        interruptModeMask = (uint8)(tempMask & 0xFFu);
    }
    else
    {
        GDU_IP_DEV_ASSERT(FALSE);
    }
