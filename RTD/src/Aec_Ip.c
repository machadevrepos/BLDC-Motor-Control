...
#define AEC_IP_AR_RELEASE_MINOR_VERSION_C       9
#define AEC_IP_AR_RELEASE_REVISION_VERSION_C    0
#define AEC_IP_SW_MAJOR_VERSION_C               7
...
#if(defined(AEC_IP_YOUNG_FELLOW) && (STD_ON == AEC_IP_YOUNG_FELLOW))
#define AEC_EVENT_STATUS_ADDR               (1U)
#define AEC_FAULT_STATUS_ADDR               (1U)
#define AEC_ADDR_FIELD_WIDTH                (12U)
#else
...
#if(defined(AEC_IP_YOUNG_FELLOW) && (STD_ON == AEC_IP_YOUNG_FELLOW))
static uint32 Aec_Ip_u32PageRegister;
#endif
...
/*FUNCTION**********************************************************************
 *
 * Function Name : Aec_Ip_SafeCastToUint8
 * Description   : Safe cast from uint32 to uint8
 *
 *END**************************************************************************/
static inline uint8 Aec_Ip_SafeCastToUint8(uint32 value)
{
    uint8 u8ReturnValue;
    if (value > 0xFFu)
    {
        AEC_DEV_ASSERT(FALSE);
        u8ReturnValue = 0;
    }
    else
    {
        u8ReturnValue = (uint8)value;
    }
                                                ...
                                                )
{
    ...
    uint8 TempDataSize;
#if(STD_ON == AEC_IP_GHS_FAULT_HANDLER)
    uint8 IsrPrio;
    uint32 GHSFault;
#endif
    ...
    if(AEC_STATUS_ERROR != Status)
    {
        ...
        if((0 != GHSFault) && (0!= Aec_GHSRouteConfig.AecGHSFaults))
        {
            ...
            for (IsrPrio = 0U; IsrPrio < AEC_IP_GHS_NUM_MAX; IsrPrio++)
            {
...
#endif
    if (Aec_Ip_u32FrameCounter >= (AEC_FRAME_COUNT_MAX - 1U))
    {
        Aec_Ip_u32FrameCounter = 1U;
    }
    else
    {
                                            ...
                                            )
{
    ...
    uint32 tempFrameSize = (uint32)DataWidth + AEC_SPI_FRAME_HEADER + AEC_SPI_FRAME_TAIL;

    FrameSize = Aec_Ip_SafeCastToUint8(tempFrameSize);
    ...
    if (FALSE == SpiConfig->SyncTransmit)
    {
        ...
        SpiStatus = Lpspi_Ip_AsyncTransmit(ExternalDevice,
                                           ...
                                           N8bWords,
                                           NULL_PTR);
        if ((AEC_STATUS_SUCCESS != Aec_Ip_WaitForAsyncTransmitEnd(ExternalDevice->Instance, AEC_IP_TIMEOUT_VALUE_US)) || (LPSPI_IP_STATUS_SUCCESS != SpiStatus))
        {
            Status = AEC_STATUS_ERROR;
        }
    }
...
Aec_Ip_StatusType  Aec_Ip_Init(const Aec_Ip_ConfigType *AecConfig)
{
    ...
    AEC_DEV_ASSERT((boolean)(AecConfig           != NULL_PTR));
    ...
    Aec_StartTimeout(&StartTime, &ElapsedTime, &TimeoutTicks, AEC_IP_TIMEOUT_VALUE_US);
                                  ...
                                  )
{
    ...
    AEC_DEV_ASSERT((boolean)(Aec_apxInternalCfg           != NULL_PTR));
    AEC_DEV_ASSERT((boolean)(Aec_apxInternalCfg->AeSpiConfig           != NULL_PTR));
    AEC_DEV_ASSERT((boolean)((DataWidth == 8U) || (DataWidth == 16U) || (DataWidth== 32U)));
    AEC_DEV_ASSERT((boolean)(RegAddr < 1024U));
    AEC_DEV_ASSERT((boolean)(Data != NULL_PTR));
...
#if(defined(AEC_IP_YOUNG_FELLOW) && (STD_ON == AEC_IP_YOUNG_FELLOW))
Aec_Ip_StatusType Aec_Ip_SpiWritePage (uint32 RegAddr,
                                      uint8 DataWidth,
                                      uint32 Data
                                      )
{
    if ((RegAddr & (~((1<<(AEC_ADDR_FIELD_WIDTH-1))-1))) != Aec_Ip_u32PageRegister) {
        Aec_Ip_SpiWrite(((1<<AEC_ADDR_FIELD_WIDTH)-1), RegAddr & (~((1<<(AEC_ADDR_FIELD_WIDTH-1))-1)), 32);
        Aec_Ip_u32PageRegister = RegAddr & (~((1<<(AEC_ADDR_FIELD_WIDTH-1))-1));
    }
    return Aec_Ip_SpiWrite(RegAddr & ((1<<(AEC_ADDR_FIELD_WIDTH-1))-1), DataWidth, Data);
}

Aec_Ip_StatusType Aec_Ip_SpiReadPage (uint32 RegAddr,
                                       uint8 DataWidth,
                                       uint32 *Data
                                       )
{
    ...
    return Aec_Ip_SpiRead(RegAddr & ((1<<(AEC_ADDR_FIELD_WIDTH-1))-1), DataWidth, Data);
}
#endif
...
uint16 Aec_Ip_DecodeGlobalHealthStatus(void)
{
    ...
    uint8 Index;
    uint16 Status;
    uint16 Aec_Ip_u32GlobalHealthTemp = (uint16)(Aec_Ip_u32GlobalHealth & 0xFFFFUL);

    Status = ((uint16)Aec_Ip_u32GlobalHealthTemp & AEC_STATUS_GLOBAL_HEALTH_MASK);
                                ...
                                )
{
    ...
    *ActualFrameCount = (uint8)((Aec_Ip_u32GlobalHealthTemp & AEC_SPI_FRAME_COUNT_MASK) >> AEC_SPI_FRAME_COUNT_SHIFT);
