...
#define AEC_IP_HW_ACCESS_AR_RELEASE_MINOR_VERSION_C       9
#define AEC_IP_HW_ACCESS_AR_RELEASE_REVISION_VERSION_C    0
#define AEC_IP_HW_ACCESS_SW_MAJOR_VERSION_C               7
...
#define AEC_EVENT_EN_ADDR                   (1U)
#define AEC_FAULT_STATUS_ADDR               (1U)
#define AEC_FAULT_EN_ADDR                   (1U)
#else
...
#define AEC_IP_UINT32_MAX_VALUE                          (0xFFFFFFFFU)
/*==================================================================================================
*                                         LOCAL CONSTANTS
==================================================================================================*/
...
void Aec_StartTimeout(uint32 *StartTimeOut,
                       ...
                       uint32 TimeoutUs)
{
    *StartTimeOut    = OsIf_GetCounter(AEC_IP_TIMEOUT_TYPE);
    *ElapsedTimeOut  = 0U;
    *TimeoutTicksOut = OsIf_MicrosToTicks(TimeoutUs, AEC_IP_TIMEOUT_TYPE);
}
                            ...
                            uint32 TimeoutTicks)
{
    ...
    uint32 u32Elapsed = OsIf_GetElapsed(StartTimeInOut, AEC_IP_TIMEOUT_TYPE);

    /* Prevent overflow before addition */
    if ((AEC_IP_UINT32_MAX_VALUE - *ElapsedTimeInOut) < u32Elapsed)
    {
        /* Assign to maximum value */
        *ElapsedTimeInOut = AEC_IP_UINT32_MAX_VALUE;
    }
    else
    {
...
void Aec_IRQEventFaultHandler(void)
{
    ...
    if (0U != HealthStatus)
    {
        for (IsrPrio = 0U; IsrPrio < AEC_IP_EVENT_FAULT_NUM_MAX; IsrPrio++)
        {
            /* Event/Fault will be handled from low to high */
            for (Index = 0U; Index < AEC_IP_EVENT_FAULT_NUM_MAX; Index++)
            {
                if (IsrPrio == Aec_intRouteConfig.aIntConfig[Index].IsrPriority)
                {
                    ...
                    {
                        #if(STD_ON == AEC_IP_EVENT_FAULT_FLAG_CLEAR)
